    ...
    try:
        with db.begin():
            ...
            if lesson.start_time - datetime.utcnow() < timedelta(hours=24):
                raise HTTPException(status_code=400, detail="Derse 24 saat kala rezervasyon yapılamaz")

            max_capacity = (
                1 if lesson.lesson_type == LessonType.private
                else lesson.room.capacity
            )
        ...
        raise HTTPException(status_code=400, detail="Bu derste zaten aktif bir rezervasyonunuz var")
    ...
    try:
        with db.begin():
            if not reservation.is_active:
                raise HTTPException(status_code=400, detail="Rezervasyon zaten iptal edilmis")

            if reservation.lesson.start_time - datetime.utcnow() < timedelta(hours=24):
                raise HTTPException(status_code=400, detail="24 saat kala rezervasyon iptali yapilamaz")
            reservation.is_active = False
            reservation.deleted_at = datetime.utcnow()

            credit = (
                db.query(StudentCredit).filter(
                    StudentCredit.user_id == student_id,
                    reservation.lesson.lesson_type == StudentCredit.lesson_type).with_for_update().one()
            )

            if not credit:
                raise HTTPException(status_code=404, detail="Ogrenci kredisi bulunamadi")
